...
try:
    ...
    MPI = None
from baselines.ppo2_context.runner import Runner
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        if np.random.uniform()>0.3:
            runner = Runner(env=env, model=model, nsteps=nsteps, gamma=gamma, lam=lam,c=context, population=population, ent_pool_coef=ent_pool_coef, ent_version=ent_version,embeddings=embeddings)
        else:
            runner = Runner(env=env, model=model, nsteps=nsteps, gamma=gamma, lam=lam,c=real_c, population=population, ent_pool_coef=ent_pool_coef, ent_version=ent_version,embeddings=embeddings)
        obs, c, other_obs,other_act, returns, masks, actions, values, neglogpacs, states, other_obs_tensor,other_act_tensor,epinfos = runner.run() #pylint: disable=E0632
        ...
        other_obs=np.reshape(other_obs,(-1)).tolist()
        other_act=np.reshape(other_act,(-1)).tolist()
