...
from overcooked_ai_py.agents.agent import AgentFromPolicy_eval,AgentFromPolicy_context
...
class Runner(AbstractEnvRunner):
    ...
    - Make a mini batch
    """
    def __init__(self, *, env, model, nsteps, gamma, lam, population=None, ent_pool_coef=0.0, ent_version=3, history_len=10,c=None,embeddings=None):
    ...
    def run(self):
        ...
        def other_agent_action():
            if self.env.use_action_method:
                ...
                if type(self.env.other_agent) is AgentFromPolicy_context:
                    c=np.array([[1,0,0,0] for _ in range(30)]) #sp
                    other_agent_actions = self.env.other_agent.direct_policy(self.obs1,c=c)
                elif type(self.env.other_agent) is AgentFromPolicy_eval:
        ...
        for _ in range(self.nsteps):
            ...
            if overcooked:
                ...
                actions, values, self.states, neglogpacs, action_probs_agent0 = self.model.step(observation=self.obs0,c=self.c,S=self.states, M=self.dones)
