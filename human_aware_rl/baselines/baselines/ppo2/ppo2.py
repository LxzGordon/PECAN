...
def learn_eval(*, network, env, total_timesteps, early_stopping = False, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, ent_pool_coef=0.0, ent_version=1, lr=3e-4, vf_coef=0.5,  max_grad_norm=0.5, gamma=0.99, lam=0.95, log_interval=10, nminibatches=4, noptepochs=4, cliprange=0.2, save_interval=0, load_path=None, model_fn=None, population=None, metric_np=None, scope='', **network_kwargs):
    '''
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        # Get minibatch
        obs, returns, masks, actions, values, neglogpacs, states, epinfos = runner.run_eval() #pylint: disable=E0632
...
def learn(*, network, env, total_timesteps, early_stopping = False, eval_env = None, seed=None, nsteps=2048, 
            ent_coef=0.0, ent_pool_coef=0.0, ent_version=1, lr=3e-4, vf_coef=0.5,  max_grad_norm=0.5, gamma=0.99, lam=0.95,
            log_interval=10, nminibatches=4, noptepochs=4, cliprange=0.2, save_interval=0, load_path=None, model_fn=None, 
            population=None, metric_np=None, scope='',
            w=None,ens_population=None,c=None,c_identifier=None,
            **network_kwargs):
    '''
    Learn policy using PPO algorithm (https://arxiv.org/abs/1707.06347)
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    obs_buffer=deque(maxlen=3)
    act_buffer=deque(maxlen=3)
    print("TOT NUM UPDATES", nupdates) # 3
    context=np.random.uniform(-0.1,0.1,size=(30,4))
    context[:,2]+=3
    context=th.softmax(th.Tensor(context),1).detach().cpu().numpy()
    for update in range(1, nupdates+1):
        assert nbatch % nminibatches == 0, "Have {} total batch size and want {} minibatches, can't split evenly".format(nbatch, nminibatches)
        # Start timer
        tstart = time.perf_counter()
        ...
        if np.random.uniform()>0.3:
            runner = Runner(env=env, model=model, nsteps=nsteps, gamma=gamma, lam=lam, population=population, ens_population=ens_population,w=w,ent_pool_coef=ent_pool_coef, ent_version=ent_version,c=context)
        else:
            runner = Runner(env=env, model=model, nsteps=nsteps, gamma=gamma, lam=lam, population=population, ens_population=ens_population,w=w,ent_pool_coef=ent_pool_coef, ent_version=ent_version,c=real_c)
        obs,c,mb_other_obs,mb_other_action, returns, masks, actions, values, neglogpacs, states, other_obs_tensor,other_act_tensor,epinfos = runner.run() #pylint: disable=E0632
        
        obs_buffer.append(other_obs_tensor[:,:10].view(30,1,10,other_obs_tensor.shape[2],other_obs_tensor.shape[3],other_obs_tensor.shape[4]).cuda())
        act_buffer.append(other_act_tensor[:,:10].view(30,1,10,1).cuda())

        o_context=th.cat([o for o in obs_buffer],dim=1)
        other_a=th.cat([a for a in act_buffer],dim=1).view(-1,1)
        t=th.zeros((30*len(act_buffer)*10,6)).cuda()
        a_context=t.scatter_(dim=1,index=other_a.to(th.int64).cuda(),src=th.ones(t.shape[0],1).cuda()).view(30,len(act_buffer),10,6)


        predict_c=c_identifier(o_context,a_context,pbt_train=True)

        context=th.softmax(predict_c,1).view(30,4).detach().cpu().numpy()

        other_obs=np.reshape(mb_other_obs,(-1)).tolist()
        other_act=np.reshape(mb_other_action,(-1)).tolist()
        ...
        if states is None: # nonrecurrent version
            ...
            for _ in range(noptepochs):
                ...
                for start in tqdm.trange(0, nbatch, nbatch_train, desc="{}/{}".format(_, noptepochs)):
                    ...
                    mbinds = inds[start:end]
                    slices = (arr[mbinds] for arr in (obs,c, returns, masks, actions, values, neglogpacs))
                    mblossvals.append(model.train(lrnow, cliprangenow, *slices))
