...
class Runner(AbstractEnvRunner):
    ...
    - Make a mini batch
    """
    def __init__(self, *, env, model, nsteps, gamma, lam, population=None,ens_population=None,w=None,c=None, ent_pool_coef=0.0, ent_version=3, history_len=10):
        ...
        self.ens_population=ens_population
        self.w=w
        self.c=c
    ...
    def run_eval(self):
        ...
        for _ in range(self.nsteps):
            ...
            if overcooked:
                ...
                actions, values, self.states, neglogpacs, action_probs_agent0 = self.model.step(self.obs0, S=self.states, M=self.dones)
    ...
    def run(self):
        # Here, we init the lists that will contain the mb of experiences
        mb_obs, mb_rewards, mb_actions, mb_values, mb_dones, mb_neglogpacs = [],[],[],[],[],[]
        other_obs_tensor,other_act_tensor=[],[]
        mb_other_obs,mb_other_action=[],[]
        mb_states = self.states
        epinfos = []
        # For n in range number of steps
        ...
        def other_agent_action():
            if self.env.use_action_method:
                ...
                if len(self.ens_population)>1:
                    ens_probs=[]
                    for a in self.ens_population:
                        ens_probs.append(a.direct_action(self.obs1))
                    ens_probs=np.transpose(np.array(ens_probs),(1,0,2))
                    w=np.repeat(self.w.reshape(1,1,-1),ens_probs.shape[0],0)
                    probs=np.matmul(w,ens_probs).reshape(-1,6)
                    other_agent_actions=[np.random.choice(len(Action.ALL_ACTIONS), p=action_probs/action_probs.sum()) for action_probs in probs]
                    return other_agent_actions
                else:
                    c=np.array([[1,0,0,0] for _ in range(30)])
                    return self.ens_population[0].direct_policy(self.obs1,c=c)
        ...
        for _ in range(self.nsteps):
            ...
            if overcooked:
                ...
                actions, values, self.states, neglogpacs, action_probs_agent0 = self.model.step(self.obs0, c=self.c,S=self.states, M=self.dones)
                ...
                mb_obs.append(self.obs0.copy())
                other_obs=np.reshape(self.obs1[0],(1,self.obs0.shape[1],self.obs0.shape[2],self.obs0.shape[3]))
                other_obs_tensor.append(th.Tensor(self.obs1).view(30,1,self.obs0.shape[1],self.obs0.shape[2],self.obs0.shape[3]))
                mb_other_obs.append(other_obs.copy())
                other_act=np.reshape(other_agent_actions[0],(1,1))
                other_act_tensor.append(th.Tensor(other_agent_actions).view(30,1,1))
                mb_other_action.append(other_act.copy())
            else:
                actions, values, self.states, neglogpacs = self.model.step(self.obs, S=self.states, M=self.dones)
                mb_obs.append(self.obs.copy())
        ...
        #batch of steps to batch of rollouts
        mb_obs = np.asarray(mb_obs, dtype=self.obs.dtype)
        mb_other_obs = np.asarray(mb_other_obs, dtype=self.obs.dtype)
        mb_c=np.repeat(np.reshape(self.c,(1,-1,4)),mb_obs.shape[0],axis=0)
        mb_rewards = np.asarray(mb_rewards, dtype=np.float32)
        mb_actions = np.asarray(mb_actions)
        mb_other_action=np.asarray(mb_other_action)
        mb_values = np.asarray(mb_values, dtype=np.float32)
        mb_neglogpacs = np.asarray(mb_neglogpacs, dtype=np.float32)
        mb_dones = np.asarray(mb_dones, dtype=np.bool)
        last_values = self.model.value(self.obs,self.c, S=self.states, M=self.dones)
        ...
        other_obs_tensor=th.cat([obs for obs in other_obs_tensor],dim=1)
        other_act_tensor=th.cat([act for act in other_act_tensor],dim=1)
        return (*map(sf01, (mb_obs,mb_c,mb_other_obs,mb_other_action, mb_returns, mb_dones, mb_actions, mb_values, mb_neglogpacs)),
            mb_states, other_obs_tensor,other_act_tensor,epinfos)
