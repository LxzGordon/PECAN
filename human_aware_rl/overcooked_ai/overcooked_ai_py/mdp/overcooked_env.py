...
class OvercookedEnv(object):
    ...
    def run_agents_context(self, agent_pair, include_final_state=False, display=False, display_until=np.Inf):
        """
    ...
    def get_rollouts_context(self, agent_pair, num_games, display=False, final_state=False, agent_idx=0, reward_shaping=0.0, display_until=np.Inf, info=True):
        """
        ...
        for _ in tqdm.trange(num_games):
            ...
            trajectory, time_taken, tot_rews_sparse, tot_rews_shaped = self.run_agents_context(agent_pair, display=display, include_final_state=final_state, display_until=display_until)
    ...
    def run_agents_eval(self, agent_pair, include_final_state=False, display=False, display_until=np.Inf):
        """
        Trajectory returned will a list of state-action pairs (s_t, joint_a_t, r_t, done_t).
        """
        assert self.cumulative_sparse_rewards == self.cumulative_shaped_rewards == 0, \
    ...
    def get_rollouts_eval(self, agent_pair, num_games, display=False, final_state=False, agent_idx=0, reward_shaping=0.0, display_until=np.Inf, info=True):
        """
        Simulate `num_games` number rollouts with the current agent_pair and returns processed 
        trajectories.
        ...
        for _ in tqdm.trange(num_games):
            ...
            trajectory, time_taken, tot_rews_sparse, tot_rews_shaped = self.run_agents_eval(agent_pair, display=display, include_final_state=final_state, display_until=display_until)
            obs, actions, rews, dones = trajectory.T[0], trajectory.T[1], trajectory.T[2], trajectory.T[3]
            trajectories["ep_observations"].append(obs)
            trajectories["ep_actions"].append(actions)
            trajectories["ep_rewards"].append(rews)
    ...
    def get_rollouts_identify_c(self,agent1,agent2,num_games=1, c_identifier=None,embeddings=None):
        import torch as th
        from collections import deque
        from overcooked_ai_py.agents.agent import AgentPair_context,AgentFromPolicy_context
        obs_buffer1=deque(maxlen=3)
        act_buffer1=deque(maxlen=3)
        obs_buffer2=deque(maxlen=3)
        act_buffer2=deque(maxlen=3)
        res=[]
        act_dict={
            (0, -1):0,
            (0, 1):1,
        ...
        context=np.random.uniform(-0.1,0.1,size=(1,4))
        context[:,2]+=1
        context[:]/=context[:].sum()
        ...
        context[:,2]+=1
        context[:]/=context[:].sum()
        agent2.context=context
        
        for i in range(num_games):

            agent1_frompolicy=agent1.get_agent(eval=True)
            agent2_frompolicy=agent2.get_agent(eval=True)
            agent_pair=AgentPair_context(agent1_frompolicy,agent2_frompolicy)
            traj=self.get_rollouts_context(agent_pair, 1,info=False)
            res.append(traj)
            
            if type(agent1_frompolicy) is AgentFromPolicy_context:
                other_idx=1
                states=traj['ep_observations']
                obs=th.Tensor([self.mdp.lossless_state_encoding(s)[other_idx] for s in states[0]]).cuda()[:10].unsqueeze(0)
                obs_buffer1.append(obs)
                act=[a[other_idx] for a in traj["ep_actions"][0]]
                act=[act_dict[a] for a in act]
                action=th.zeros((400,6)).scatter_(1,th.LongTensor(act).view(400,1),th.ones(400,1)).cuda()[:10].unsqueeze(0)
                act_buffer1.append(action)

                observations=th.cat([obs for obs in obs_buffer1],dim=0)
                actions=th.cat([act for act in act_buffer1],dim=0)

                c=th.softmax(c_identifier(observations,actions,eval=True).view(-1),dim=0).detach().cpu().numpy()
                agent2.context=c

            if type(agent2_frompolicy) is AgentFromPolicy_context:
                other_idx=0
                states=traj['ep_observations']
                obs=th.Tensor([self.mdp.lossless_state_encoding(s)[other_idx] for s in states[0]]).cuda()[:10].unsqueeze(0)
                obs_buffer2.append(obs)
                act=[a[other_idx] for a in traj["ep_actions"][0]]
                act=[act_dict[a] for a in act]
                action=th.zeros((400,6)).scatter_(1,th.LongTensor(act).view(400,1),th.ones(400,1)).cuda()[:10].unsqueeze(0)
                act_buffer2.append(action)

                observations=th.cat([obs for obs in obs_buffer2],dim=0)
                actions=th.cat([act for act in act_buffer2],dim=0)
