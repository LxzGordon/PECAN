...
from overcooked_ai_py.agents.agent import AgentPair,AgentGroup_ens, CoupledPlanningAgent, RandomAgent, GreedyHumanModel
...
#from human_aware_rl.imitation.behavioural_cloning import ImitationAgentFromPolicy
...
class AgentEvaluator(object):
    ...
    def evaluate_agent_pair_eval(self, agent_pair, num_games=1, display=False, info=True):
        return self.env.get_rollouts_eval(agent_pair, num_games, display=display, info=info)
        
    def evaluate_agent_pair_context(self, agent1,agent2, num_games=1, c_identifier=None,display=False, info=True):
        if c_identifier is None:
            agent_pair=AgentPair_context(agent1,agent2,allow_duplicate_agents=type(agent1)==type(agent2))
            traj=self.env.get_rollouts_context(agent_pair, num_games, display=display, info=info)
            return [traj]

        #initialize context
        if type(agent1)is not AgentFromPolicy_context:
            agent1.set_context(np.array([0,1,0,0]))
            other_idx=0
        if type(agent2)is not AgentFromPolicy_context:
            agent2.set_context(np.array([0,1,0,0]))
            other_idx=1
        ...
        for _ in range(3):
            score_buffer.append(0)
        for i in range(num_games):
            agent_pair=AgentPair_context(agent1,agent2,allow_duplicate_agents=type(agent1)==type(agent2))
            traj=self.env.get_rollouts_context(agent_pair, 1, display=display, info=info)
            res.append(traj)
            score_buffer.append(traj['ep_returns'])

            states=traj['ep_observations']
            obs=th.Tensor([self.env.mdp.lossless_state_encoding(s)[other_idx] for s in states[0]]).cuda()[:10].unsqueeze(0)
            ...
            observations=th.cat([obs for obs in obs_buffer],dim=0)
            actions=th.cat([act for act in act_buffer],dim=0)

            c=th.softmax(c_identifier(observations,actions,eval=True).view(-1),dim=0).detach().cpu().numpy()
            
            if type(agent1)is not AgentFromPolicy_context:
                agent1.set_context(c)
            if type(agent2)is not AgentFromPolicy_context:
                agent2.set_context(c)
        return res

    def evaluate_agent_pair_ens(self, agent0, agent_1, num_games=1, display=False, info=True):
        return self.env.get_rollouts_ens(agent0,AgentGroup_ens([agent_1]),np.array([1]), num_games, display=display)
