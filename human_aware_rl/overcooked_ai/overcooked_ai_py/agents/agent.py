...
class AgentGroup(object):
    ...
    def __init__(self, agents, allow_duplicate_agents=True):
        self.agents = agents
        try:
...
class AgentGroup_ens(object):
    ...
    def joint_action(self, state,w,ind=None):
        try:
            if ind:
                ens_probs=[a.state_policy(state, a.mdp, 1, True, True) for i,a in enumerate(self.agents)]
                probs=np.matmul(w,ens_probs).reshape(-1,6)
                act_dict=[(0, -1), (0, 1), (1, 0), (-1, 0), (0, 0), 'interact']
                a0=act_dict[np.random.choice(6,p=probs)]
            else:
                act_dict=[(0, -1), (0, 1), (1, 0), (-1, 0), (0, 0), 'interact']
                tuple(act_dict(np.random.choice(6,p=a.action(state))) for a in self.agents)
        except:
            a0=self.agents[0].action(state)
...
class AgentPair(AgentGroup):
    ...
    def __init__(self, *agents, allow_duplicate_agents=False): 
        ...
        super().__init__([self.a0,self.a1])
        self.n == 2
...
class AgentFromPolicy(Agent):
    ...
    def __init__(self, state_policy, direct_policy, stochastic=True, action_probs=True):
        """
...
class AgentPair_eval(AgentGroup_eval):
    """
    AgentPair is the N=2 case of AgentGroup. Unlike AgentGroup,
    it supports having both agents being the same instance of Agent.
...
class AgentFromPolicy_eval(Agent):
    """
    Defines an agent from a `state_policy` and `direct_policy` functions
    """
...
class AgentFromPolicy_context(Agent):
    """
    Defines an agent from a `state_policy` and `direct_policy` functions
    """
    ...
    def action(self, state,partner_context):
        """
        The standard action function call, that takes in a Overcooked state
        and returns the corresponding action.
        ...
        self.history.append(state)
        try:
            return self.state_policy(state,partner_context, self.mdp, self.agent_index, self.stochastic, self.action_probs)
        except AttributeError as e:
            raise AttributeError("{}. Most likely, need to set the agent_index or mdp of the Agent before calling the action method.".format(e))
...
class AgentGroup_context(object):
    """
    AgentGroup is a group of N agents used to sample 
    joint actions in the context of an OvercookedEnv instance.
    """
    ...
    def joint_action(self, state):
        ...
        for i,a in enumerate(self.agents):
            if type(a) is AgentFromPolicy_eval:
                act.append(a.action(state))
            elif type(a) is AgentFromPolicy_context:
                act.append(a.action(state,self.agents[1-i].context))
            else:
                act.append(a.action(state,self.agents[1-i].context))
        return tuple(a for a in act)
...
class AgentPair_context(AgentGroup_context):
    """
    AgentPair is the N=2 case of AgentGroup. Unlike AgentGroup,
    it supports having both agents being the same instance of Agent.
    ...
    def joint_action(self, state):
        if self.a0 is self.a1:
            ...
            # reset agent index at each turn to prevent overwriting it
            self.a0.set_agent_index(0)
            action_0 = self.a0.action(state,self.a1.context)
            self.a1.set_agent_index(1)
            action_1 = self.a1.action(state,self.a0.context)
            return (action_0, action_1)
        else:
            return super().joint_action(state)