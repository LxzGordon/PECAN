...
from overcooked_ai_py.agents.agent import AgentFromPolicy_eval,AgentFromPolicy,AgentFromPolicy_context,AgentFromPolicy_context, AgentPair, RandomAgent
from human_aware_rl.context.test2 import Net
...
from baselines.ppo2.ppo2 import learn,learn_eval
from baselines.ppo2_context.ppo2 import learn as learn_context
from baselines.common.vec_env import VecEnvWrapper
from baselines.common.vec_env.subproc_vec_env import SubprocVecEnv
from baselines.common.models import register
from baselines.common.policies import PolicyWithValue,PolicyWithValue_context
...
class LinearAnnealer():
    ...
    def param_value(self, timestep):
        ...
        gamma=max(4 - (timestep / (2*self.horizon)), 1)
        assert 0 <= curr_value <= 1
        return curr_value,gamma
...
def conv_network_fn(**kwargs):
    ...
    def network_fn(X,partner_type):
        ...
        out = tf.layers.flatten(conv_out)
        out=tf.concat([out,partner_type],axis=1)
        print(out.shape)
        for _ in range(num_hidden_layers):
            out = tf.layers.dense(out, size_hidden_layers, activation=tf.nn.leaky_relu)
...
def get_pbt_agent_from_config(save_dir=None, sim_threads=0, seed=0, agent_idx=0, best=False, agent_to_load_path=None,iter=None):
...
def get_pbt_agent_from_config_eval(save_dir=None, sim_threads=0, seed=0, agent_idx=0, best=False, agent_to_load_path=None,iter=None):
    if agent_to_load_path is None:
        agent_folder = save_dir + 'seed_{}/agent{}'.format(seed, agent_idx)
        if best:
            agent_to_load_path = agent_folder  + "/best"
            ...
            agent_to_load_path = agent_folder  + "/pbt_iter" + str(iter)
    agent = get_agent_from_saved_model_eval(agent_to_load_path, sim_threads)
    return agent

def get_agent_from_saved_model_eval(save_dir, sim_threads):
    """Get Agent corresponding to a saved model"""
    # NOTE: Could remove dependency on sim_threads if get the sim_threads from config or dummy env
    state_policy, processed_obs_policy = get_model_policy_from_saved_model(save_dir, sim_threads)
    return AgentFromPolicy_eval(state_policy, processed_obs_policy)

def get_pbt_agent_from_config_context(save_dir=None, sim_threads=0, seed=0, agent_idx=0, best=False, agent_to_load_path=None,iter=None):
    if agent_to_load_path is None:
        agent_folder = save_dir + 'seed_{}/agent{}'.format(seed, agent_idx)
        if best:
            agent_to_load_path = agent_folder  + "/best"
            ...
            agent_to_load_path = agent_folder  + "/pbt_iter" + str(iter)
    agent = get_agent_from_saved_model_context(agent_to_load_path, sim_threads)
    return agent

def get_agent_from_saved_model_context(save_dir, sim_threads):
    """Get Agent corresponding to a saved model"""
    # NOTE: Could remove dependency on sim_threads if get the sim_threads from config or dummy env
    state_policy, processed_obs_policy = get_model_policy_from_saved_model_context(save_dir, sim_threads)
    return AgentFromPolicy_context(state_policy, processed_obs_policy)

def get_agent_from_model_context(model, c, sim_threads, is_joint_action=False):
    """Get Agent corresponding to a loaded model"""
    state_policy, processed_obs_policy = get_model_policy_from_model_context(model, sim_threads, is_joint_action=is_joint_action)
    p=AgentFromPolicy_context(state_policy, processed_obs_policy)
    p.context=c
    return p 

def get_agent_from_model(model,c, sim_threads, is_joint_action=False,eval=False):
    """Get Agent corresponding to a loaded model"""
    state_policy, processed_obs_policy = get_model_policy_from_model(model, sim_threads, is_joint_action=is_joint_action,eval=eval)
    if not eval:
        return AgentFromPolicy(state_policy, processed_obs_policy)
    else:
        p=AgentFromPolicy_eval(state_policy, processed_obs_policy)
        p.context=c
        return p 
...
def get_model_policy_from_saved_model_context(save_dir, sim_threads):
    """Get a policy function from a saved model"""
    predictor = tf.contrib.predictor.from_saved_model(save_dir)
    step_fn = lambda obs,c: predictor({"obs": obs,'c':np.reshape(c,(30,16))})["action_probs"]
    return get_model_policy_context(step_fn, sim_threads)

def get_model_policy_from_model(model, sim_threads, is_joint_action=False,eval=False):
    def step_fn(obs):
        action_probs = model.act_model.step(obs, return_action_probs=True)
        return action_probs
    return get_model_policy(step_fn, sim_threads, is_joint_action=is_joint_action,eval=eval)

def get_model_policy_from_model_context(model, sim_threads, is_joint_action=False):
    def step_fn(obs,c=None):
        if type(model.act_model)==PolicyWithValue:
            action_probs = model.act_model.step(obs, return_action_probs=True)
        elif type(model.act_model)==PolicyWithValue_context:
            action_probs = model.act_model.step(obs, c,return_action_probs=True)
        else: 
            assert(ValueError)
        return action_probs
    return get_model_policy_context(step_fn, sim_threads, is_joint_action=is_joint_action)

def get_model_policy(step_fn, sim_threads, is_joint_action=False,eval=False):
    """
    ...
    """
    def encoded_state_policy(observations, stochastic=True, return_action_probs=True):
...
def get_model_policy_context(step_fn, sim_threads, is_joint_action=False):
    """
    Returns the policy function `p(s, index)` from a saved model at `save_dir`.
    ...
    step_fn: a function that takes in observations and returns the corresponding
             action probabilities of the agent
    """
    def encoded_state_policy(observations, c=None, stochastic=True, return_action_probs=False):
        """Takes in SIM_THREADS many losslessly encoded states and returns corresponding actions"""
        if c is not None:
            action_probs_n = step_fn(observations,c)
        else:
            action_probs_n = step_fn(observations)
        if return_action_probs:
            return action_probs_n
    ...
    def state_policy(mdp_state,partner_context, mdp, agent_index, stochastic=True, return_action_probs=False):
        """Takes in a Overcooked state object and returns the corresponding action"""
        obs = mdp.lossless_state_encoding(mdp_state)[agent_index]
        padded_obs = np.array([obs] + [np.zeros(obs.shape)] * (sim_threads - 1))
        partner_context=np.repeat(np.reshape(partner_context,(1,16)),sim_threads,axis=1)
        action_probs = step_fn(padded_obs,partner_context)[0] # Discards all padding predictions
...
def create_model_context(env, agent_name, use_pretrained_weights=False, **kwargs):
    """Creates a model and saves it at a location
    
    env: a dummy environment that is used to determine observation and action spaces
    agent_name: the scope under which the weights of the agent are saved
    """
    model, _ = learn_context(
        network='conv_and_mlp_context', 
        env=env, 
        total_timesteps=1, 
        save_interval=0, 
        nsteps=kwargs["BATCH_SIZE"],
...
def update_model_context(env, model, population=None, ent_version=1, metric_np=None,c=None,c_identifier=None,embeddings=None, **kwargs):
    """
    ...
    def model_fn(**kwargs):
        return model
    #c_identifier=Net()
    updated_model, run_info = learn_context(
        network=kwargs["NETWORK_TYPE"], 
        env=env, 
...
def update_model(env, model, population=None, ent_version=1, metric_np=None,ens_population=None,w=None,c=None,c_identifier=None,embeddings=None,**kwargs):
    """
    Train agent defined by a model using the specified environment.
...
def update_model_eval(env, model, population=None, ent_version=1, metric_np=None, **kwargs):
    """
    Train agent defined by a model using the specified environment.
    ...
    updated_model, run_info = learn_eval(
        network=kwargs["NETWORK_TYPE"], 
        env=env, 
        total_timesteps=kwargs["PPO_RUN_TOT_TIMESTEPS"],
        nsteps=kwargs["BATCH_SIZE"],
